# DL Country Framework v0.9.10

Static dashboard for country-level equity expected returns across 22 ETFs, with a Vercel serverless function fetching live commodity prices from Yahoo Finance.
...
├── api/
│   ├── commodities.py      ← serverless function — Yahoo Finance via yfinance (batched)
...
Live moves from `/api/commodities` — a Vercel Python function that calls `yfinance.download()` with a single batched request for all five tickers:

| Commodity | Yahoo ticker |
|---|---|
| Brent oil | `BZ=F` |
| Copper | `HG=F` |
| Iron ore proxy | `VALE` (Vale common; spot iron ore on Yahoo is unreliable) |
| Agriculture | `DBA` |
| Broad commodity | `DBC` |

The function returns both `m6_pct` (6-month % change, drives arrow signal) and `yoy_pct` (12-month % change, display context). Cached 1 hour at the CDN. If a fetch fails for a ticker, the UI falls back to manual input for that one.

Numerical commodity impacts on returns remain zeroed; the panel drives only the qualitative Commodity Impact arrow per country.
...
1. **Push to GitHub:**
   ```bash
   git add .
   git commit -m "v0.9.10: Yahoo Finance batched fetch"
   git push
   ```
...
3. **Verify the function** by visiting `your-app.vercel.app/api/commodities` — should return JSON with five commodities, each with `price`, `yoy_pct`, `m6_pct`, and `ok: true`.
